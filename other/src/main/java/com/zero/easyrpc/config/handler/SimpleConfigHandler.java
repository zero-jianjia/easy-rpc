...
import com.zero.easyrpc.proxy.RefererInvocationHandler;
import com.zero.easyrpc.registry.motan.Registry;
import com.zero.easyrpc.registry.motan.RegistryFactory;
import com.zero.easyrpc.rpc.*;
import com.zero.easyrpc.rpc.protocol.ProtocolFilterDecorator;
...
//    }
    ...
    public <T> T refer(Class<T> interfaceClass, List<Cluster<T>> clusters, String proxyType) {
        ProxyFactory proxyFactory = null;//ExtensionLoader.getExtensionLoader(ProxyFactory.class).getExtension(proxyType);
    ...
    public <T> Exporter<T> export(Class<T> interfaceClass, T ref, List<URL> registryUrls) {
        ...
        String serviceStr = null;// = StringTools.urlDecode(registryUrls.get(0).getParameter(URLParamType.embed.getName()));
        URL serviceUrl = URL.valueOf(serviceStr);

        // export service
        // 利用protocol decorator来增加filter特性
        String protocolName = null;// serviceUrl.getParameter(URLParamType.protocol.getName(), URLParamType.protocol.getValue());
        Protocol orgProtocol =null;// ExtensionLoader.getExtensionLoader(Protocol.class).getExtension(protocolName);
...
//            LoggerUtil.warn("Exception when unexport exporters:" + exporters);
    ...
    private void register(List<URL> registryUrls, URL serviceUrl) {
        ...
        for (URL url : registryUrls) {
            ...
            RegistryFactory registryFactory = null;// ExtensionLoader.getExtensionLoader(RegistryFactory.class).getExtension(url.getProtocol());
...
//                                + ", make sure registry module for " + url.getProtocol() + " is in classpath!"));
    ...
    private void unRegister(Collection<URL> registryUrls) {
        for (URL url : registryUrls) {
            ...
            try {
                String serviceStr =null;// StringTools.urlDecode(url.getParameter(URLParamType.embed.getName()));
                URL serviceUrl = URL.valueOf(serviceStr);
