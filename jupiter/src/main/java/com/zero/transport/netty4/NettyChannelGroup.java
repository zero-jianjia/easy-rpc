...
import org.zero.common.atomic.AtomicUpdater;
...
public class NettyChannelGroup implements ChannelGroup {
    ...
    // 连接断开时自动被移除
    private final ChannelFutureListener REMOVER_LISTENER = new ChannelFutureListener() {
    ...
    public boolean add(Channel channel) {
        ...
        if (added) {
            ...
            ((NettyChannel) channel).channel().closeFuture().addListener(REMOVER_LISTENER);
            deadlineMillis = -1;
